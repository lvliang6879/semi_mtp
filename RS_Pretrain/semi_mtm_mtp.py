...
from dataset.semi_mtm_mtp import SemiDataset
...
from model.ordet.multi_modal_task_model import MultiModalTask
...
def det_collate_fn_unsup(batch):
    """用于无监督数据：(img_w, img_s, sar_img, ignore_masks, target)"""
    img_w, img_s, sar_img, ignore_masks, targets = zip(*batch)
    img_w = torch.stack(img_w, dim=0)
    img_s = torch.stack(img_s, dim=0)
    sar_img = torch.stack(sar_img, dim=0)
    ignore_masks = torch.stack(ignore_masks, dim=0)   # [B,1,H,W]
    return img_w, img_s, sar_img, ignore_masks, list(targets)
...
def main():
    ...
    model = MultiModalTask(args, cfg).cuda()
    ...
    for epoch in range(epoch + 1, total_epochs):
        ...
        for i, ((img_x, mask_x, target),
                (img_u_w, img_u_s, sar_img, ignore_mask, target_u)) in enumerate(zip(trainloader_l, trainloader_u)):
            
            t0 = time.time()
            img_x, mask_x = img_x.cuda(), mask_x.cuda()
            img_u_w, img_u_s, sar_img, ignore_mask = img_u_w.cuda(), img_u_s.cuda(), sar_img.cuda(), ignore_mask.cuda()
            ...
            with torch.no_grad():
                model.eval()
                pseudo_ds, u_w_out = model(img_u_w, data_samples_u, modality='opt')
            ...
            with torch.cuda.amp.autocast(enabled=amp):
                loss_sup_dict, x_out = model(img_x, data_samples, modality='opt')   # ★ loss 在模型内部
                loss_opt_dict, u_s_out = model(img_u_s, pseudo_gt_samples, modality='opt')
                loss_sar_dict, sar_out = model(sar_img, pseudo_gt_samples, modality='sar')
                # loss_total = sum_loss_dict(loss_dict)
                loss_sup_rd = parse_losses(loss_sup_dict['loss_rd'])
                loss_opt_rd = parse_losses(loss_opt_dict['loss_rd'])
                loss_sar_rd = parse_losses(loss_sar_dict['loss_rd'])
                loss_sup_ss = criterion_l(x_out, mask_x)
                loss_opt_ss = criterion_u(u_s_out, mask_u_w)
                loss_sar_ss = criterion_u(sar_out, mask_u_w)
                loss_opt_ss = confidence_weighted_loss(loss_opt_ss, conf_u_w, ignore_mask, conf_thresh=conf_thresh)
                loss_sar_ss = confidence_weighted_loss(loss_sar_ss, conf_u_w, ignore_mask, conf_thresh=conf_thresh)
                ...
                total_loss = (loss_sup_rd + alpha * loss_opt_rd + alpha * loss_sar_rd + loss_sup_ss + 0.5 * loss_opt_ss + 0.5 * loss_sar_ss) / 4.0
            ...
            log_info = {
                ...
                "loss_sup_rd": loss_sup_rd.detach().item(),      # supervised loss
                "loss_opt_rd": alpha * loss_opt_rd.detach().item(),  # unsupervised (pseudo-label) loss
                "loss_sar_rd": alpha * loss_sar_rd.detach().item(),  # unsupervised (pseudo-label) loss
                "loss_sup_ss": loss_sup_ss.detach().item(),      # supervised loss
                "loss_opt_ss": 0.5 * loss_opt_ss.detach().item(),  # unsupervised (pseudo-label) loss
                "loss_sar_ss": 0.5 * loss_sar_ss.detach().item(),  # unsupervised (pseudo-label) loss
...
def validation(args, cfg, logger, iters, model, val_loader):
    ...
    with torch.no_grad():
        for imgs, seg_targets, targets in pbar:
            ...
            outputs, seg_logits = model(imgs, data_samples, modality='opt')  # dict: {'output_rd', 'output_seg'}
