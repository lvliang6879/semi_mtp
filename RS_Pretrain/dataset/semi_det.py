...
class SemiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Train augmentations
        img, boxes = resize_det(img, boxes, (0.5, 2.0))     # 前面已经写好的 resize_det
        img, boxes = crop_det(img, boxes, self.size)
        img, boxes = hflip_det(img, boxes, p=0.5)
        img, boxes = bflip_det(img, boxes, p=0.5)
        ...
        if self.mode == 'train_l':
            return normalize_det(img), target
        ...
        return normalize_det(img_w), normalize_det(img_s), target