...
class SelfAttentionBlock(nn.Module):
    def __init__(
        ...
        if self.use_adapter:
            ...
            self.adapter_ffn = AdapterFFN(
                self.mlp,
                copy_weights=adapter_cfg.get("copy_weights", True),
                zero_init_out=adapter_cfg.get("zero_init_out", True),
    ...
    def _forward_list(self, x_list: List[Tensor], rope_list=None) -> List[Tensor]:
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            if self.use_adapter and self.adapter_ffn is not None:
                residual_2_list = [r + self.adapter_ffn(n) for r, n in zip(residual_2_list, norm2_list)]
            ...
            for x, rope in zip(x_list, rope_list):
                ...
                ffn_out = self.mlp(n2)
                if self.use_adapter and self.adapter_ffn is not None:
                    ffn_out = ffn_out + self.adapter_ffn(n2)
                x_ffn = x_attn + self.ls2(ffn_out)
                x_out.append(x_ffn)
            x_ffn = x_out
...
class DinoVisionTransformerAdapter(nn.Module):
    def __init__(
        self,
        *,
        img_size: int = 224,
        ...
        **ignored_kwargs,
        ...
        if lora_cfg is not None:
            use_adapter = bool(lora_cfg.get("use_adapter", True))
            adapter_cfg = lora_cfg.get("adapter_cfg", {})
        ...
        if freeze_backbone:
            freeze_module(self)
            # only unfreeze adapters
            mark_only_adapter_trainable(self)
        
        tr, tot, ratio = count_trainable_params(self)
        print(f"[DinoViT-LoRA] trainable params: {tr}/{tot} ({ratio:.2f}%)")
    ...
    def init_weights(self):
        ...
        # pretrained 为 None / "" / 非字符串 -> 不加载
        if pretrained is None:
            print("[init_weights] self.pretrained is None, skip loading pretrained backbone.")
            return
        if not isinstance(pretrained, str) or pretrained.strip() == "":
            print(f"[init_weights] invalid self.pretrained={pretrained}, skip loading pretrained backbone.")
            return

        # 3) 加载 checkpoint（兼容不同保存格式）
        ckpt = torch.load(pretrained, map_location="cpu")
        state_dict = ckpt.get("model", ckpt)

        # 4) 只加载 encoder 权重到当前 backbone
        new_state_dict = {}
        for k, v in state_dict.items():
            # ckpt keys: module.encoder.xxx 或 encoder.xxx
            if k.startswith("module.encoder."):
                new_k = k.replace("module.encoder.", "", 1)
                new_state_dict[new_k] = v
            elif k.startswith("encoder."):
                new_k = k.replace("encoder.", "", 1)
                new_state_dict[new_k] = v
            else:
                # 其他 key（decoder/head/optimizer 等）不加载
                continue

        # 5) 加载
        msg = self.load_state_dict(new_state_dict, strict=False)
        print(f"[init_weights] Loaded backbone pretrained from: {pretrained}")
        print(f"[init_weights] loaded_keys={len(new_state_dict)}, "
    ...
    #         for k, v in state_dict.items():
    #             if k.startswith('module.encoder.'):
    #                 new_k = k.replace('module.encoder.', '', 1)  # 只替换第一个匹配
    #                 new_state_dict[new_k] = v
    #             else:
    #                 # 如果你不希望保留其他无关 key（如 head 等），可以跳过
    #                 # 或者保留以兼容 strict=False
    #                 new_state_dict[k] = v
...
class AdapterFFN(nn.Module):
    """
    Trainable FFN adapter parallel to the frozen FFN.
    Supports Mlp(fc1/fc2) and SwiGLUFFN(w1/w2/w3).

    Key stability trick:
      - copy_weights=True copies pretrained FFN weights
      - zero_init_out=True zeros last projection so adapter output is 0 at init
    """
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(ffn, Mlp):
            self.kind = "mlp"
            self.fc1 = nn.Linear(ffn.fc1.in_features, ffn.fc1.out_features, bias=ffn.fc1.bias is not None)
            self.act = nn.GELU()
            self.fc2 = nn.Linear(ffn.fc2.in_features, ffn.fc2.out_features, bias=ffn.fc2.bias is not None)

            if copy_weights:
                with torch.no_grad():
                    self.fc1.weight.copy_(ffn.fc1.weight)
                    if ffn.fc1.bias is not None and self.fc1.bias is not None:
                        self.fc1.bias.copy_(ffn.fc1.bias)
                    self.fc2.weight.copy_(ffn.fc2.weight)
                    if ffn.fc2.bias is not None and self.fc2.bias is not None:
                        self.fc2.bias.copy_(ffn.fc2.bias)
        ...
        elif isinstance(ffn, SwiGLUFFN):
            self.kind = "swiglu"
            self.w1 = nn.Linear(ffn.w1.in_features, ffn.w1.out_features, bias=ffn.w1.bias is not None)
            self.w2 = nn.Linear(ffn.w2.in_features, ffn.w2.out_features, bias=ffn.w2.bias is not None)
            self.w3 = nn.Linear(ffn.w3.in_features, ffn.w3.out_features, bias=ffn.w3.bias is not None)

            if copy_weights:
                with torch.no_grad():
                    self.w1.weight.copy_(ffn.w1.weight)
                    if ffn.w1.bias is not None and self.w1.bias is not None:
                        self.w1.bias.copy_(ffn.w1.bias)
                    self.w2.weight.copy_(ffn.w2.weight)
                    if ffn.w2.bias is not None and self.w2.bias is not None:
                        self.w2.bias.copy_(ffn.w2.bias)
                    self.w3.weight.copy_(ffn.w3.weight)
                    if ffn.w3.bias is not None and self.w3.bias is not None:
                        self.w3.bias.copy_(ffn.w3.bias)
            ...
            raise TypeError(f"Unsupported FFN type for AdapterFFN: {type(ffn)}")
...
def DINOV3_ViT_B_Adapter(args):
    backbone = DinoVisionTransformerAdapter(
        img_size=224,
        patch_size=16,
        in_chans=3,
        pos_embed_rope_base=100,
