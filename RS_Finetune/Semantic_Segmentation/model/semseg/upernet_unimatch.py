...
# from model.backbone.vit_win_rvsa_v3_wsz7 import vit_b_rvsa, vit_l_rvsa
# from model.backbone.intern_image import InternImage
# from model.backbone.vit import ViT_B, ViT_L
...
class UperNet(torch.nn.Module):
    ...
    def forward(self, x, need_fp=False):
        ...
        if need_fp:
            dropout_layer = nn.Dropout2d(0.5, inplace=False)
            e_dropout = tuple(dropout_layer(feature) for feature in e)
            ss = self.semsegdecoder.decode_head._forward_feature(e)
            out = self.semseghead(ss)
            out = F.interpolate(out, size=(h, w), mode="bilinear", align_corners=True)

            ss_fp = self.semsegdecoder.decode_head._forward_feature(e_dropout)
            out_fp = self.semseghead(ss_fp)
